(function () {
  ...
  window.incs = function () {
    ...
    var rows = incidents.slice().reverse().map(function (item) {
      ...
      return '<tr' + (on ? ' style="background:#13243c"' : '') + '><td>' + fmt(item.created_at) + '</td><td><b>' + esc(item.title) + '</b><div class="mono muted">' + esc(item.id) + '</div></td><td>' + esc(item.service) + '</td><td>' + esc(sourceName(item.source)) + '</td><td>' + badge(item.status) + '</td><td><button onclick="selectIncidentForAI(\'' + esc(item.id) + '\')">选择</button> <button class="primary" onclick="runIncidentAIDiagnosis(\'' + esc(item.id) + '\')" ' + (diagnosing ? 'disabled' : '') + '>AI诊断</button> <button onclick="selected=\'' + esc(item.id) + '\';go(\'trace\')">证据</button></td></tr>';
    }).join('');
